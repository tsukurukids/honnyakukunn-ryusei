...
for (const k in existing_words) {
    existing_words[k] = existing_words[k].concat(new_words[k]);
}

let output_str = "const quizWords = {\n";
for (const k in existing_words) {
    output_str += `    ${k}: [\n`;
    for (const item of existing_words[k]) {
        output_str += `        { en: ${JSON.stringify(item.en)}, ja: ${JSON.stringify(item.ja)} },\n`;
    }
...
fs.writeFileSync("script_update.txt", output_str, "utf8");
console.log("Generated script_update.txt");
